...
const handleEdit = ($event: any, item: any) => {
    ...
    formState.value.type = queryParams.value.type;
    console.log(formState.value.icon);
...
//图标上传
const handleRemoveIconChange = (info: any) => {
    console.log(info);
    formState.value.icon = "";
    // const status = info.file.status;
    // const file = info.file;
...
<template>
    <ListContainer>
        ...
        <template #table="{ height }">
            <a-table :row-key="(record: any) => record.id" size="small" :columns="columns" :data-source="tableData"
                :expand-row-by-click="true" :scroll="{ y: height }" :pagination="false">
                <template #bodyCell="{ column, record }">
                    ...
                    <template v-if="column.dataIndex === 'operation'">
                        <a-space :gutter="8">
                            <a-button size="small" type="primary" @click="handleAddSub($event, record)">添加子菜单</a-button>
        ...
        <a-modal v-model:visible="addModalStatus" :title="!formState.parentId ? '新增菜单' : '新增子菜单'" centered
            ...
            <a-form :colon="false" :model="formState" :rules="rules" :label-col="{ span: 4 }">
                ...
                <template v-if="JSON.stringify(curPermission) !== '{}'">
                    <template v-if="formState.type === 'MOBILE'">
                        <a-form-item label="android地址" v-bind="validateInfos.androidPath">
                            <a-input v-model:value="formState.androidPath" placeholder="请输入菜单地址"></a-input>
                        </a-form-item>
                        <a-form-item label="ios地址" v-bind="validateInfos.iosPath">
                            <a-input v-model:value="formState.iosPath" placeholder="请输入菜单地址"></a-input>
                        </a-form-item>
                    </template>
                    <template v-else>
                ...
                <a-form-item label="排序" v-bind="validateInfos.sort">
                    <a-input-number style="width: 100%" v-model:value="formState.sort" placeholder="请输入排序">
                    </a-input-number>
                </a-form-item>
                <a-form-item label="菜单图标" name="icon">
                    <a-upload @change="handleUploadIconChange" :data="data" action="http://bt-static.yndth.cn/upload"
                        v-model:file-list="fileList" list-type="picture-card" accept=".jpg,.png,.jpeg">
                        <plus-outlined v-if="formState.icon === ''" />
                    </a-upload>
                </a-form-item>
            </a-form>
        </a-modal>
        <a-modal v-model:visible="editModdalStatus" title="编辑菜单" :after-close="handleReset" centered width="620px"
            ...
            <a-form :colon="false" :model="formState" :rules="rules" :label-col="{ span: 4 }">
                ...
                <a-form-item label="排序" v-bind="validateInfos.sort">
                    <a-input-number v-model:value="formState.sort" style="width: 100%" placeholder="请输入排序">
                    </a-input-number>
                </a-form-item>
                <a-form-item label="菜单图标" name="icon">
                    <a-upload @remove="handleRemoveIconChange" @change="handleUploadIconChange" :data="data" action="http://bt-static.yndth.cn/upload"
                        v-model:file-list="fileList" list-type="picture-card" accept=".jpg,.png,.jpeg">
                        <plus-outlined v-if="formState.icon === ''" />
                    </a-upload>
                </a-form-item>

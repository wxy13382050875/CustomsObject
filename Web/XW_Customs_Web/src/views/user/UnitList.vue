...
import { unitColumns } from "@/interface/IUser";
import { entrustOrgList } from "@/api/user";
...
//用户列表分页服务
const { data, run, loading } = usePagination<any>(entrustOrgList, {
    manual: true,
    loadingDelay: 1000
});
    ...
    <ListContainer @refresh="getData">
        ...
        <template #table="{ height }">
            <div :style="{ height: height + 'px' }">
                <a-table size="small" :columns="unitColumns" :data-source="tableData" :loading="loading" :pagination="false"
                    :scroll="{ x: '100%', y: height  + 'px' }">

...
import { saveUserInfo, putUserInfo } from "@/api/user";
...
const params = router.currentRoute.value.query // params参数跳转
//表单对象
const formState = ref({
...
const onFinish = () => {
    ...
    if (JSON.stringify(params) === "{}") {
        saveUserInfo(formState.value).then(() => {
            message.success("添加成功");
            showContainerRef.value.refresh();
        });
    } else {

        putUserInfo(formState.value.id, formState.value).then(() => {
            message.success("添加成功");
            showContainerRef.value.refresh();
        });
    }
...
<template>
    <ShowContainer ref="showContainerRef" :title="title">
        <a-form ref="formRef" :colon="false" :model="formState" :rules="rules" name="basic" :label-col="{ span: 2 }"
            :wrapper-col="{ span: 8 }" autocomplete="off" allow-clear @finish="onFinish" @finish-failed="onFinishFailed"
            ...
            <a-form-item label="所属机构" name="orgIds">
                <a-tree-select v-model:value="formState.orgIds" has-feedback style="width: 100%" show-search
                    :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }" placeholder="请选择所属机构" allow-clear
                    tree-default-expand-all multiple :tree-data="orgTreeData"
            ...
            <a-form-item has-feedback label="手机号" name="phone">
                <a-input-number v-model:value="formState.phone" style="width: 100%" placeholder="请输入手机号" />
            </a-form-item>
            ...
            <a-form-item :wrapper-col="{ span: 14, offset: 2 }">
                ...
                <a-button style="margin-left: 10px" @click="resetForm" v-if="params.id == null">重置</a-button>
            </a-form-item>
        </a-form>
    </ShowContainer>
</template>

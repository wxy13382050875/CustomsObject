...
@interface xw_RegisterCell ()<UITextFieldDelegate>
...
@end
@implementation xw_RegisterCell
...
-(void)xw_setupUI{
    ...
    [self.contentView addSubview:self.inspectResultLabel];
    [self.contentView addSubview:self.inspectResultTextField];
}
...
-(void)setModel:(xw_HandleQueryModel *)model{
    ...
    self.inspectResultTextField.text = _model.inspectResultName;
    [self.inspectResultTextField.rac_textSignal subscribeNext:^(NSString * _Nullable x) {
        @strongify(self);
        self.model.inspectResultName = x;
    }];
}
...
-(UILabel*)inspectResultLabel{
    if (!_inspectResultLabel) {
        _inspectResultLabel = [UILabel labelWithText:@"检测结果" WithTextColor:COLOR(@"#171717") WithNumOfLine:1 WithBackColor:nil WithTextAlignment:NSTextAlignmentLeft WithFont:15];
    }
...
-(UITextField*)inspectResultTextField{
    if(!_inspectResultTextField){
        ...
        _inspectResultTextField.textColor = COLOR(@"#171717");
        _inspectResultTextField.textAlignment = NSTextAlignmentLeft;
        _inspectResultTextField.placeholder = @"请选择检测结果";
        _inspectResultTextField.backgroundColor = COLOR(@"#F5F5F5");
        UIView* rightView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 30, 30)];
        UIImageView* imageView = [UIImageView new];
        imageView.image = IMG(@"icon_down_arrow");
        [rightView addSubview:imageView];
        ...
        _inspectResultTextField.rightView = rightView;
        _inspectResultTextField.rightViewMode = UITextFieldViewModeAlways;
        ViewRadius(_inspectResultTextField, 5);
        _inspectResultTextField.delegate = self;
        [_inspectResultTextField setTextFieldLeftPadding:10];
    }
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    ...
    if(textField == self.createTimeTextField ||
       textField == self.acceptTimeTextField ){
        ...
        [self showPick];
    } else if(textField == self.inspectResultTextField){
        [self showResultPick];
    }
    return NO;
}
...
-(void)showResultPick{
    NSArray* array = [xw_DataParsingHelper getDictPickList:@"INSPECT_RESULT"];
    Dialog()
   .wTypeSet(DialogTypePickSelect)
   .wListDefaultValueSet(array[0][@"name"])  //默认
   .wEventOKFinishSet(^(id anyID, id otherData) {
          NSLog(@"%@",anyID[@"item"]);
       ChildrenModel* model = anyID[@"item"];
       self.inspectResultTextField.text = model.name;
       self.model.inspectResult = model.code;
    })
   //一层直接传入带字典/字符串的数组 name为显示的文字 其他携带的model可以自由传入
   .wDataSet(array)
   .wStart();

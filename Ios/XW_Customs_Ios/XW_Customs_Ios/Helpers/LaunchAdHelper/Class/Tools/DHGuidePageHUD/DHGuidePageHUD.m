...
//                [startButton setImage:IMG(@"experience_btn") forState:UIControlStateNormal];
                [startButton setTitle:@"立即体验" forState:UIControlStateNormal];
                [startButton setTitleColor:COLOR(@"#ffffff") forState:UIControlStateNormal];
                [startButton setBackgroundColor:RGBA_COLOR(22, 63, 132, 0.5)];
                [startButton addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
                [imageView addSubview:startButton];

                startButton.sd_layout.centerXEqualToView(imageView)
                .bottomSpaceToView(imageView, 80 )
                .widthIs(195).heightIs(48);
                
                ViewBorderRadius(startButton, 24, 1, COLOR(@"#ffffff"));
            }
        }
        
        // 设置引导页上的页面控制器
//        self.imagePageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(DDScreenW*0.0, DDScreenH*0.85, DDScreenW*1.0, DDScreenH*0.1)];
...
- (instancetype)dh_initWithFrame:(CGRect)frame videoURL:(NSURL *)videoURL {
    if ([super initWithFrame:frame]) {
        ...
        [movieStartButton setTitle:@"立即体验" forState:UIControlStateNormal];

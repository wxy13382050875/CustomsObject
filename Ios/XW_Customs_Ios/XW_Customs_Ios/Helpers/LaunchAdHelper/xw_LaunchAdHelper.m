...
@interface xw_LaunchAdHelper ()<XHLaunchAdDelegate>
@property (nonatomic, strong) LaunchAdViewModel *viewModel;
@property (nonatomic, strong) NSArray *array;
@end
@implementation xw_LaunchAdHelper
+(void)load{
    [self shareManager];
}

+(xw_LaunchAdHelper *)shareManager{
    static xw_LaunchAdHelper *instance = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken,^{
        instance = [[xw_LaunchAdHelper alloc] init];
    });
    return instance;
}
...
-(void)setupXHLaunchAd{
    ...
    [[self.viewModel.requestCommand execute: params] subscribeNext:^(id responseObject) {
//        GGLog(@"%@",[NSString DataTOjsonString:responseObject]);
//        LaunchAdModel* model = [LaunchAdModel mj_objectWithKeyValues:responseObject[@"data"]];
        self.array = [LaunchAdModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"]];
        if (self.array.count > 0) {
            LaunchAdModel* model =self.array[0];
            if ([model.adCreativeForm.resourcesType isEqualToString:@"veido"]) {
                [self exampleVedio:model];
            } else {
...
-(void)exampleImage:(LaunchAdModel*)model{
    ...
    imageAdconfiguration.imageNameOrURLString = model.resourcesUrl;
    ...
    imageAdconfiguration.openModel = model.jumpTargetPage;
    //广告显示完成动画
    ...
    if([XHLaunchAd checkImageInCacheWithURL:[NSURL URLWithString:model.resourcesUrl]]){
        //设置要添加的自定义视图(可选)
...
//视频开屏广告 - 网络数据
-(void)exampleVedio:(LaunchAdModel*)model{
    ...
    videoAdconfiguration.videoNameOrURLString = model.resourcesUrl;
    //是否关闭音频
    ...
    //广告点击打开页面参数(openModel可为NSString,模型,字典等任意类型)
    videoAdconfiguration.openModel = model.jumpTargetPage;
    //广告显示完成动画
    ...
    if([XHLaunchAd checkVideoInCacheWithURL:[NSURL URLWithString:model.resourcesUrl]]){
        //设置要添加的自定义视图(可选)
...
-(LaunchAdViewModel*)viewModel{
    if (!_viewModel) {
        _viewModel = [[LaunchAdViewModel alloc] init];
    }
    return _viewModel;
}
/**
